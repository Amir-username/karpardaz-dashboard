...
function EmployerRequestsPage() {
  ...
  const handleSearchRequests = () => {
    const fetchRequests = async () => {
      const res = await fetch(
        BASE_LINK + `advertise-requests/${Number(searchInput)}`
      );
  ...
  return (
    <div className="flex flex-col gap-10 items-center justify-center w-full h-full m-32 ">
      <h1 className="text-2xl">لیست درخواست های آگهی کارفرما</h1>
        ...
        <div className="rounded-lg shadow-sm ring-1 ring-gray-200">
          <Table className="p-2">
            <TableRow className="flex w-full font-bold p-2">
              ...
              <TableCell className="w-64">شناسه کارجو</TableCell>
            ...
            {requests.map((req) => (
              <TableRow className="flex w-full p-2" key={req.id}>
                ...
                <TableCell className="w-64">{req.jobseeker_id}</TableCell>
